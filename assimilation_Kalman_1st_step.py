...
class TGrid:
    ...
    def __init__(self, tau_left=5500., tau_right=9000., 
...
def ObsOperator(Grid, Rays):
    ...
    for Ray in Rays:
        ...
        t_in = tau_rec
        h_in = hdown
        it_prev = trunc((t_in - tleft) / dtau)   # trunc отбрасывает значение после запятой 
        ih_prev = trunc((h_in - hdown) / dheight) #  и конвертирует из float в int
