...
class TRay:
   ...
   def __init__(self, tay_rec, height_sat, betta):
   ...
   def tay_from_height(self, height):
       return self.tay_rec + R * (np.radians(self.betta) - np.arccos(R * np.cos(np.radians(self.betta)) / (R + height)))
...
class TGrid:
    ...
    def __init__(self, tay_left=5500., tay_right=9000., 
                 height_down=0., height_up=1000., tay_nodes=101, height_nodes=33):
...
class Background:
    ...
    def zerostate(self, tay, height):
        ...
        ne = NeQuick_2.nequick(height, np.rad2deg(tay/R), self.lon, self.month, self.f10p7, self.hour)
    ...
    def irregularity(self, tay, height, N_ir_max, h0, tay0, a, b, gamma, v):
        ...
        alpha = ((height - h0) * np.cos(np.radians(gamma)) - (tay - tay0) * np.sin(np.radians(gamma)))**2. / a**2. + \
                ((height - h0) * np.sin(np.radians(gamma)) + (tay - tay0) * np.cos(np.radians(gamma)))**2. / b**2.
        if alpha <= 1.:
    ...
    def model(self, tay, height):
        ...
        for N_ir_max, h0, tay0, a, b, gamma, v in zip(self.N_ir_max_arr, self.h0_arr, self.tay0_arr, self.a_arr, self.b_arr, self.gamma_arr, self.v_arr):
            if v == 1.:
                result = result + self.irregularity(tay, height, N_ir_max, h0, tay0, a, b, gamma, v)
            if v == 0.:
                result = result - self.zerostate(tay, height) * self.irregularity(tay, height, N_ir_max, h0, tay0, a, b, gamma, v)
        if result <= 0.:
            result = 0.
...
def CreateRays(rec_tays, Grid):
    ...
    for rec_tay in rec_tays:
        for b in np.arange(5,175.5, 0.5):  # Ширина лучей приёмника, было arange(5, 180, 0.5)
            if(TRay(rec_tay, R_sat - R, b).tay_from_height(Grid.height_up) < Grid.tay_right and TRay(rec_tay, R_sat - R, b).tay_from_height(Grid.height_up) > Grid.tay_left):
                Rays.append(TRay(rec_tay, R_sat - R, b))
    '''
...
def ComputeRHS(Rays, Model):
    ...
    def f1(height, Ray, Model): # Интеграл для вычисления ПЭС с учётом Якобиана преобразования
        return Model.model(Ray.tay_from_height(height), 
...
def ObsOperator(Grid, Rays):
    ...
    def f(h, Ray, t0, h0, dt, dh):
        ...
        return J * (h - h0) * (Ray.tay_from_height(h) - t0) / (dt * dh)
...
def main():
    ...
    Model = Background(month=6, hour=12, f10p7=90, lon=20,
                 ...
                 tay0_arr=[6000., 6500., 7125., 8000., 8700.], a_arr=[60., 60., 60., 60.,60.],
    ...
    MyGrid = TGrid()
    rec_tays = [6200, 6900, 7500, 8200, 8900] # Координата приёмников
    Rays = CreateRays(rec_tays, MyGrid)
    ...
    for j in np.arange(MyGrid.height_nodes):
        for i in np.arange(MyGrid.tay_nodes):
            apriory_state[i + j * (MyGrid.tay_nodes)] = Model.zerostate(MyGrid.tay_left + i * MyGrid.dtay, MyGrid.height_down + j * MyGrid.dheight)
    ...
    Result_rshape = Result.reshape((MyGrid.height_nodes, MyGrid.tay_nodes))
    ...
    for j in np.arange(MyGrid.height_nodes):
        for i in np.arange(MyGrid.tay_nodes):
            ModelN[i + j * (MyGrid.tay_nodes)] = Model.model(MyGrid.tay_left + i * MyGrid.dtay, MyGrid.height_down + j * MyGrid.dheight)
...
###################################################################################################################################################    
    ...
    im = plt.contourf(ModelN, levels, extent=(np.rad2deg(MyGrid.tay_left/R), np.rad2deg(MyGrid.tay_right/R), MyGrid.height_down, MyGrid.height_up), extend="both", cmap=cm.jet)
    #res = plt.contourf(Result, levels, extent=(np.rad2deg(MyGrid.tay_left/R), np.rad2deg(MyGrid.tay_right/R), MyGrid.height_down, MyGrid.height_up), extend="both", cmap=cm.jet)
...
##################################################################################################################################################
    ...
    #im = plt.contourf(ModelN, levels, extent=(np.rad2deg(MyGrid.tay_left/R), np.rad2deg(MyGrid.tay_right/R), MyGrid.height_down, MyGrid.height_up), extend="both", cmap=cm.jet)
    res = plt.contourf(Result_rshape, levels, extent=(np.rad2deg(MyGrid.tay_left/R), np.rad2deg(MyGrid.tay_right/R), MyGrid.height_down, MyGrid.height_up), extend="both", cmap=cm.jet)
 ...
 ##################################################################################################################################   
    ...
    #im = plt.contourf(ModelN, levels, extent=(np.rad2deg(MyGrid.tay_left/R), np.rad2deg(MyGrid.tay_right/R), MyGrid.height_down, MyGrid.height_up), extend="both", cmap=cm.jet)
    aprState = plt.contourf(apriory_state_plot, levels, extent=(np.rad2deg(MyGrid.tay_left/R), np.rad2deg(MyGrid.tay_right/R), MyGrid.height_down, MyGrid.height_up), extend="both", cmap=cm.jet)
